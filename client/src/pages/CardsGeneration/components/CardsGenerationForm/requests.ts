...
import { CardsGenerationRequest, CardsGenerationResponse, GenerateCardsProps } from "./types";
import { deduceNewCards } from "./utils";

export const postGenerateCards = (body: CardsGenerationRequest, formData: FormData, props: GenerateCardsProps) => {
  ...
  sendRequest(RestVerb.Post, BACKEND_URL + API.CARDS_GENERATION.GENERATE_CARDS, formData)
    .then((response: CardsGenerationResponse) => {
      ...
      if (body.generateOutro === true) cardPaths.push(`${PROCESSED_CARDS_PATH}/${OUTRO_FILENAME}`);
    ...
    .catch((error: ApiResponse) => {
      if (error.status === HttpStatus.PreconditionFailed) sendToast(toasts.CardGen.NoCardsContents, ToastType.Error);
      else sendToast(error.message, ToastType.Error);
    })
    .finally(() => {
