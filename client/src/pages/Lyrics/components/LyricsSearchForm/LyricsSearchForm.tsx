...
const LyricsSearchForm = () => {
  ...
  return (
    <form
      onSubmit={(e) =>
        handleLyricsSearchSubmit(
          e,
          { artist, songName },
          { isFetching, setIsFetching, setLyricsParts, setPageMetadata, setDismissedParts }
        )
      }
      className="search-form"
    >
      <label htmlFor="artist" className="hidden">
        {"Artist"}
      </label>
      <input
        required
      ...
      <label htmlFor="songName" className="hidden">
        {"Song name"}
      </label>
      <input
        required
