...
def getVerticalOffset(font_type: MetanameFontNames) -> int:
    match font_type:
        case MetanameFontNames.LATIN:
            return 0
        case MetanameFontNames.S_CHINESE:
            return -10
        case MetanameFontNames.T_CHINESE:
            return -10
        case MetanameFontNames.JAPANESE:
            return -11
        case MetanameFontNames.KOREAN:
            return -10
        case _:
            return -1  # fallback
    return 0
...
def getCharFontType(char: str) -> MetanameFontNames:
    ...
    if char.isascii() or char in "‘’“”" or 0x0080 <= c <= 0x00FF or 0x0100 <= c <= 0x017F or 0x0180 <= c <= 0x024F:
