...
def generateSingleCard(cards_contents: CardsContents, song_data: SongMetadata, settings: CardgenSettings) -> Response:
    ...
    """
    enforce_bottom_color = str(settings.get(PayloadFields.ENFORCE_BOTTOM_COLOR))
    include_bg_img = bool(settings.get(PayloadFields.INCLUDE_BG_IMG))
    ...
    try:
        ...
        return createApiResponse(HttpStatus.PRECONDITION_FAILED, str(e))
    log.info("Card metadata calculated successfully.")

    image_output_path = f"{getUserProcessedPath()}{SLASH}{str(settings.get(PayloadFields.CARD_FILENAME)).split('/')[-1]}"
...
class SingleCardGenerationResource(Resource):
    ...
    def post(self) -> Response:
        ...
        if err or cardgen_settings is None or card_contents is None or song_data is None:
